...
export default function ProfilePage() {
  ...
  const [orderFilter, setOrderFilter] = useState<'All' | 'New' | 'Process' | 'Done'>('All'); // <-- НОВОЕ СОСТОЯНИЕ ДЛЯ ФИЛЬТРА
  ...
  // --- ЛОГИКА УРОВНЕЙ И ФИЛЬТРОВ ---
  const completedOrdersCount = incomingOrders.filter(o => o.status === 'Done').length;
  ...
  const currentLevel = getSellerLevel(completedOrdersCount);

  // Фильтруем заказы для отображения
  const filteredOrders = incomingOrders.filter(o => orderFilter === 'All' || o.status === orderFilter);
  ...
  return (
    <div className="min-h-screen bg-[#F3F4F6] font-sans text-[#333]">
      ...
      <main className="max-w-[1240px] mx-auto px-4 py-8">
        {/* PROFILE CARD */}
        <div className="bg-white rounded-[12px] p-6 shadow-sm mb-8 flex flex-col md:flex-row items-center gap-6 relative overflow-hidden">
           {/* Декоративный фон уровня */}
           <div className={`absolute -right-10 -top-10 w-40 h-40 opacity-10 rounded-full blur-2xl ${currentLevel.style.split(' ')[0]}`}></div>
           
           <div className="w-[100px] h-[100px] shrink-0 rounded-full bg-emerald-50 border border-emerald-100 flex items-center justify-center text-4xl text-[#11a95e] font-black shadow-inner z-10">
           ...
           <div className="flex-1 text-center md:text-left z-10">
             <div className="flex flex-col md:flex-row md:items-center gap-3 mb-1">
                ...
                {/* РАНГ ПРОДАВЦА */}
                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[12px] font-black border shadow-sm ${currentLevel.style}`}>
                  <span className="text-sm">{currentLevel.icon}</span> {currentLevel.text}
                </span>
             </div>
             ...
             <div className="flex flex-wrap justify-center md:justify-start gap-3 sm:gap-4 text-[13px]">
                ...
                <div className="bg-gray-50 px-4 py-2 rounded-[8px] border border-gray-100">{translate('orders_done')} <b className="text-[#11a95e]">{completedOrdersCount}</b></div>
             </div>
           </div>
           
           <button onClick={openNewServiceModal} className="bg-[#11a95e] text-white px-6 py-3 rounded-[8px] font-bold shadow-md hover:bg-[#0e9552] transition-colors shrink-0 z-10">
        ...
        <div className="flex flex-wrap border-b border-gray-200 mb-8 overflow-x-auto hide-scrollbar">
          <button onClick={() => setActiveTab('services')} className={`px-5 sm:px-6 py-3 font-bold whitespace-nowrap transition-colors ${activeTab === 'services' ? 'border-b-2 border-[#11a95e] text-[#11a95e]' : 'text-gray-500 hover:text-gray-700'}`}>{translate('tab_services')}</button>
          <button onClick={() => setActiveTab('orders')} className={`px-5 sm:px-6 py-3 font-bold whitespace-nowrap transition-colors ${activeTab === 'orders' ? 'border-b-2 border-[#11a95e] text-[#11a95e]' : 'text-gray-500 hover:text-gray-700'}`}>{translate('tab_orders')} {incomingOrders.filter(o => o.status === 'New').length > 0 && <span className="ml-2 bg-red-500 text-white text-[10px] px-2 py-0.5 rounded-full">{incomingOrders.filter(o => o.status === 'New').length}</span>}</button>
          <button onClick={() => setActiveTab('settings')} className={`px-5 sm:px-6 py-3 font-bold whitespace-nowrap transition-colors ${activeTab === 'settings' ? 'border-b-2 border-[#11a95e] text-[#11a95e]' : 'text-gray-500 hover:text-gray-700'}`}>{translate('tab_settings')}</button>
        </div>
        ...
        {/* Вкладка: ЗАКАЗЫ */}
        {activeTab === 'orders' && (
          <div className="bg-white rounded-[12px] shadow-sm p-6 border border-gray-100">
            ...
            <div className="flex flex-wrap gap-2 mb-6 pb-6 border-b border-gray-100">
               {['All', 'New', 'Process', 'Done'].map(f => (
                 ...
                 >
                   {translate(`filter_${f.toLowerCase()}`)} 
            ...
            {filteredOrders.length === 0 ? (
              ...
              <div className="overflow-x-auto rounded-[8px] border border-gray-200">
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-gray-50 text-[11px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-200">
                      ...
                      <React.Fragment key={o.id}>
                        <tr className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                          <td className="p-4">
                            ...
                            <div className="text-[12px] text-gray-500 font-medium">{o.client_email}</div>
                          </td>
                          <td className="p-4">
                            <span className={`px-2.5 py-1 rounded-[4px] text-[10px] font-black uppercase tracking-wider shadow-sm ${getStatusStyle(o.status)}`}>{o.status}</span>
                          </td>
                          <td className="p-4 text-right">
                            <div className="flex justify-end gap-2 items-center">
                              <button 
                                onClick={() => setOpenChatOrderId(openChatOrderId === o.id ? null : o.id)}
                                className={`px-3 py-1.5 rounded-[6px] font-bold text-[12px] transition-colors border ${
                              ...
                              {o.status === 'Process' && <button onClick={() => updateOrderStatus(o.id, 'Done')} className="px-3 py-1.5 bg-[#11a95e] hover:bg-[#0e9552] text-white rounded-[6px] font-bold text-[12px] transition-colors shadow-sm shadow-[#11a95e]/20">{translate('btn_deliver')}</button>}
                            </div>
                          </td>
                        </tr>
                        {openChatOrderId === o.id && (
                          <tr>
                            <td colSpan={3} className="p-0 border-b border-gray-200">
                              <div className="bg-gray-50/50 p-4 shadow-inner">
